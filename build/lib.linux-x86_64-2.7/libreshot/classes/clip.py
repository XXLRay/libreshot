...
class clip:
    ...
    def __init__(self, clip_name, color, position_on_track, start_time, end_time, parent_track, file_object):
        ...
        self.rotation = 0.0
        self.thumb_location = ""
        self.shift_button1_pressed=False
    ...
    def GeneratePreviewXML(self, file_name, preview_mode="trimming"):
        ...
        bg_image = files.LibreShotFile(project)
    ...
    def RenderClip(self, item = None, x_offset = 0, y_offset = 0):
        ...
        # load clip images
        imgTrack_Middle = gtk.image_new_from_file("%s/libreshot/themes/%s/Clip_Middle_%s.png" % (self.parent.parent.project.form.libreshot_path, theme, self.color))
        imgTrack_Left = gtk.image_new_from_file("%s/libreshot/themes/%s/Clip_Left_%s.png" % (self.parent.parent.project.form.libreshot_path, theme, self.color))
        imgTrack_Right = gtk.image_new_from_file("%s/libreshot/themes/%s/Clip_Right_%s.png" % (self.parent.parent.project.form.libreshot_path, theme, self.color))
        ...
        if total_pixel_length < imgTrack_Left_Width + imgTrack_Right_Width + theme_settings["clip"]["collapse_pixel_threshold"]:
            ...
            if has_regular_images == False:
                ...
                # Load buttons
                if self.play_video:
                    imgTrack_Visible = gtk.image_new_from_file("%s/libreshot/themes/%s/visible_transparent.png" % (self.parent.parent.project.form.libreshot_path, theme))
                else:
                    imgTrack_Visible = gtk.image_new_from_file("%s/libreshot/themes/%s/not_visible_transparent.png" % (self.parent.parent.project.form.libreshot_path, theme))
            ...
            if has_regular_images == False:
                ...
                # Load buttons
                if self.play_audio:
                    imgTrack_Visible = gtk.image_new_from_file("%s/libreshot/themes/%s/speaker_transparent.png" % (self.parent.parent.project.form.libreshot_path, theme))
                else:
                    imgTrack_Visible = gtk.image_new_from_file("%s/libreshot/themes/%s/speaker_mute_transparent.png" % (self.parent.parent.project.form.libreshot_path, theme))
            ...
            if has_regular_images == False:
                ...
                # Load buttons
                if self.effects:
                    imgTrack_Visible = gtk.image_new_from_file("%s/libreshot/themes/%s/effect.png" % (self.parent.parent.project.form.libreshot_path, theme))
        ...
        if isArrow:
            ...
            if (event.state & gtk.gdk.BUTTON1_MASK):
                ...
                if self.get_valid_drop(item.get_bounds().x1 + total_x_diff, item.get_bounds().y1 + total_y_diff):
                    ...
                    # Determine SHIFT/CTRL snapping amount (if SHIFT/CTRL key is pressed)
                    if self.shift_button1_pressed:
                    ...
                    elif self.parent.parent.project.form._CTRL:
                        # CTRL is pressed
...
#                    direction = "left"
#                else:
                if event.x_root < (self.position_on_track * pixels_per_second) + imgTrack_Left_Width:
                    direction = "left"
                elif event.x_root > ((self.position_on_track + self.length()) * pixels_per_second) - imgTrack_Right_Width:
                    direction = "right"
                else:
                    direction = "none"

                # if none, update the cursor the standard icon
                if direction == "none":
                    # update cursor variable
                    if self.parent.parent.project.form.current_cursor[0] != "none":

                        # change cursor to normal
                        self.parent.parent.project.form.current_cursor[0] = "none"
            ...
            if (event.state & gtk.gdk.BUTTON1_MASK):
                ...
                side = self.parent.parent.project.form.current_cursor[0]  # left or right or none
                ...
                if side == "left":
                    ...
                    new_length = float(original_end_pos - event.x_root) / float(pixels_per_second)
                    ...
                    new_length = (event.x_root - (self.position_on_track * pixels_per_second)) / float(pixels_per_second)
        ...
        if event.button == 1:
            ...
            if isArrow:
                ...
                if event.type == gtk.gdk._2BUTTON_PRESS: 
                    ...
                    self.shift_button1_pressed = self.parent.parent.project.form._SHIFT
    ...
    def on_button_release_x (self, item, target, event):
        ...
        if isArrow:
            ...
            if event.button == 1:
                ...
                prev_pos=self.position_on_track
                ...
                if self.shift_button1_pressed:
                    shifted_by = self.position_on_track - prev_pos
                    for trk in self.parent.parent.tracks:
                        for cl in trk.clips:
                            start = float(cl.position_on_track)
                            if start >= prev_pos and cl <> self:
                                cl.position_on_track = start + shifted_by
                        for tr in trk.transitions:
                            start = float(tr.position_on_track)
                            if start >= prev_pos:
                                tr.position_on_track = start + shifted_by
                    self.parent.parent.project.form.refresh()
        elif isResize:

            # only respond to the first mouse button
            if self.parent.parent.project.form.current_cursor[0] <> "none" and event.button == 1:
