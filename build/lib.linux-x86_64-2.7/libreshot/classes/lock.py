...
def check_folders_exist(path):
	# ensure the user's ~/.libreshot directory exists
...
def check_pid(path):
	""" Check and see if this is the only instance of LibreShot. If not, then kill this instance. """
	...
	if os.path.exists(pidPath):
		...
		try:
			...
			# LibreShot is already running, so...
			is_running = True
			...
			# LibreShot is not currently running, so...
			is_running = False
		
		# LibreShot is alreay running, so kill this instance (ONLY KILL IF IT CONTAINS ARGS)
		# The user should be able to run as many instances of LibreShot as needed... but when ARGS
		...
		if len(sys.argv) > 1:
			...
			for arg in sys.argv[1:]:
				...
				if ".osp" not in arg:
					...
					# create a import queue file for the primary instance of LibreShot
			...
			# exit the program (if LibreShot is already running) and ARGV is passed in
...
class queue_watcher ( threading.Thread ):
	""" This class polls the /queue/ folder, looking for files to import into LibreShot.  When it finds
	a text file, it should get the path out of the file, import the file, and then delete the file.  Only
	1 instance of LibreShot should be polling this folder. """
	...
	def run ( self ):
		...
		# only allow this thread to run if this instance of LibreShot is the primary instance.
		...
		if os.getpid() == pid:
			...
			print "Not the primary instance of LibreShot. Not starting queue watcher thread."
