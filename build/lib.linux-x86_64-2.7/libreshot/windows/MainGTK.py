...
import subprocess
import shutil
from widgets import libreshotwidgets
...
# Main window of LibreShot
class frmMain(SimpleGtkBuilderApp):


	def __init__(self, path="Main.ui", root="frmMain", domain="LibreShot", project=None, version="0.0.0", **kwargs):
		...
		# determine the directory LibreShot is running in.  This is used 
		# to correctly load images, themes, etc...
		self.libreshot_path = self.project.BASE_DIR
		...
		self.OSTreeHistory = TreeHistory.LibreShotTree(self.treeHistory, self.project)
		# ---------------------------------
		...
		# Check for files being passed into LibreShot
		self.check_args()
	...
	def get_avformats(self, melt_command):
		...
		if self.has_frei0r_installed == False:
			# warning
			messagebox.show(_("Error"), _("Not all effects can be loaded. LibreShot can not find the frei0r effect library installed. Please install the frei0r-plugins package from your package manager.\n\nLibreShot will still continue to work, but will have less effects to choose from."))
	...
	def on_nbFiles_switch_page(self, widget, *args):
		...
		if new_page_pos == 1:
			...
			if not self.OSIcvTransitions:
				...
				self.OSIcvTransitions = IcvTransitions.LibreShotTree(self.icvTransitions, self.project)
			...
			if not self.OSTreeEffects:
				...
				# init effects tab
				self.OSTreeEffects = TreeEffects.LibreShotTree(self.icvEffects, self.project)
	...
	def check_args(self):
		""" Loop through args collection passed to LibreShot, and look for media files,
		or project files """
	...
	def refresh(self, refresh_files = True):
		...
		self.frmMain.set_title("LibreShot - %s" % (self.project.name))
	...
	def refresh_files(self, category=None, filter=None):
		...
		if mode == "treeFiles":
			...
			for item in items:
				...
				elif isinstance(item, files.LibreShotFolder):
					#add folders
	...
	def exit_libreshot_with_save(self):
		# mark as exiting
	...
	def exit_libreshot_with_no_save(self):
		# mark as exiting
		self.is_exiting = True
	...
	def on_mnuHelpContents_activate(self, widget, *args):
		...
		try:
			...
			helpfile = "ghelp:libreshot"
			screen = gtk.gdk.screen_get_default()
			gtk.show_uri(screen, helpfile, gtk.get_current_event_time())
		except:
			messagebox.show(_("Error!"), _("Unable to open the Help Contents. Please ensure the libreshot-doc package is installed."))
	...
	def on_tlbMakeMovie_clicked(self, widget, *args):
		...
		if emptytimeline:
			messagebox.show(_("Libreshot Error!"), _("The timeline is empty, there is nothing to export."))
			return
...
class mnuTrack(SimpleGtkBuilderApp):
	...
	def __init__(self, rows, selected, path="Main_track_menu.ui", root="mnuTrackPopup", domain="LibreShot", form=None, project=None, **kwargs):
		SimpleGtkBuilderApp.__init__(self, os.path.join(project.UI_DIR, path), root, domain, **kwargs)
...
class mnuMarker(SimpleGtkBuilderApp):
	...
	def __init__(self, rows, selected, path="Main_marker_menu.ui", root="mnuMarkerPopup", domain="LibreShot", form=None, project=None, **kwargs):
		SimpleGtkBuilderApp.__init__(self, os.path.join(project.UI_DIR, path), root, domain, **kwargs)
...
class mnuTransition(SimpleGtkBuilderApp):
	...
	def __init__(self, rows, selected, path="Main_transition_properties.ui", root="mnuTransitionPopup", domain="LibreShot", form=None, project=None, **kwargs):
		SimpleGtkBuilderApp.__init__(self, os.path.join(project.UI_DIR, path), root, domain, **kwargs)
	...
	def on_mnuShiftTransitions_activate (self, event, *args):
		...
		text = inputbox.input_box(title="LibreShot", message=_("Please enter the # of seconds to shift the transitions:"), default_text="5.0")
		if text:
...
class mnuFadeSubMenu(SimpleGtkBuilderApp):
	...
	def __init__(self, rows, selected, path="Main_fade_menu.ui", root="mnuFadeSubMenuPopup", domain="LibreShot", form=None, project=None, **kwargs):
		SimpleGtkBuilderApp.__init__(self, os.path.join(project.UI_DIR, path), root, domain, **kwargs)
...
class mnuRotateSubMenu(SimpleGtkBuilderApp):
	...
	def __init__(self, rows, selected, path="Main_rotate_menu.ui", root="mnuRotateSubMenuPopup", domain="LibreShot", form=None, project=None, **kwargs):
		SimpleGtkBuilderApp.__init__(self, os.path.join(project.UI_DIR, path), root, domain, **kwargs)
...
class mnuPlayheadSubMenu(SimpleGtkBuilderApp):
	...
	def __init__(self, rows, selected, path="Main_playhead_menu.ui", root="mnuPlayheadSubMenuPopup", domain="LibreShot", form=None, project=None, **kwargs):
		SimpleGtkBuilderApp.__init__(self, os.path.join(project.UI_DIR, path), root, domain, **kwargs)
...
class mnuAnimateSubMenu(SimpleGtkBuilderApp):
	...
	def __init__(self, rows, selected, path="Main_animate_menu.ui", root="mnuAnimateSubMenuPopup", domain="LibreShot", form=None, project=None, **kwargs):
		SimpleGtkBuilderApp.__init__(self, os.path.join(project.UI_DIR, path), root, domain, **kwargs)
...
class mnuPositionSubMenu(SimpleGtkBuilderApp):
	...
	def __init__(self, rows, selected, path="Main_layout_menu.ui", root="mnuPositionSubMenuPopup", domain="LibreShot", form=None, project=None, **kwargs):
		SimpleGtkBuilderApp.__init__(self, os.path.join(project.UI_DIR, path), root, domain, **kwargs)
...
class mnuClip(SimpleGtkBuilderApp):
	...
	def __init__(self, rows, selected, path="Main_clip_properties.ui", root="mnuClipPopup", domain="LibreShot", form=None, project=None, **kwargs):
		SimpleGtkBuilderApp.__init__(self, os.path.join(project.UI_DIR, path), root, domain, **kwargs)
	...
	def on_mnuShiftClips_activate (self, event, *args):
		...
		text = inputbox.input_box(title="LibreShot", message=_("Please enter the # of seconds to shift the clips:"), default_text="5.0")
		if text:
			# convert to peroid decimal
			text = text.replace(',', '.')
			try:
...
class mnuTree(SimpleGtkBuilderApp):
	...
	def __init__(self, rows, selected, path="Main_tree_popup.ui", root="mnuTreePopUp", domain="LibreShot", form=None, project=None, **kwargs):
		SimpleGtkBuilderApp.__init__(self, os.path.join(project.UI_DIR, path), root, domain, **kwargs)
...
class frmFolders(SimpleGtkBuilderApp):
	...
	def __init__(self, path="Main_folder_menu.ui", root="frmFolder", domain="LibreShot", form=None, project=None, **kwargs):
		SimpleGtkBuilderApp.__init__(self, os.path.join(project.UI_DIR, path), root, domain, **kwargs)
		...
		if getIconPath("libreshot"):
			self.FolderDlg.set_icon_from_file(getIconPath("libreshot"))
