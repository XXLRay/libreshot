...
class Settings:
	...
	#The first time Libreshot is run, the settings file won't exist,
	...
	def save_settings_to_xml(self):
		...
		if os.path.exists(settings_path):
			try:
				...
				messagebox.show(_("LibreShot Warning"), _("Invalid or empty preferences file found, loaded default values"))
				xmldoc = xml.Document()
	...
	def write_to_settings_file(self, xmldoc):
		...
		try:
			...
			messagebox.show(_("LibreShot Error"), _("Unexpected Error '%s' while writing to '%s'." % (inst, filename)))
